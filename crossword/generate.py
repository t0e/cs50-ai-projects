...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        for word1 in self.domains[x]:
            ...
            if not overlap_possible:
                domain_copy.remove(word1)
                revised = True

        self.domains[x] = domain_copy
        return revised
    ...
    def ac3(self, arcs=None):
        ...
        """
        if arcs == None:
            arcs = set(self.crossword.overlaps)
    ...
    def order_domain_values(self, var, assignment):
        ...
        """
        unassigned_neighbors = self.crossword.neighbors(
            var) - set(assignment.keys())

        constrain_counter = {}

        for word in self.domains[var]:
            counter = 0
            for variable in unassigned_neighbors:
                overlap = self.crossword.overlaps[var, variable]
                for neighbor_word in self.domains[variable]:
                    if word[overlap[0]] != neighbor_word[overlap[1]]:
                        counter += 1
            constrain_counter[word] = counter
        result = sorted(constrain_counter, key=constrain_counter.get)
        return result
