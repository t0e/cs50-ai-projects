...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        if count == 0:
            for c in nearby_cells:
                self.mark_safe(c)
        elif count == len(nearby_cells):
            for c in nearby_cells:
                self.mark_mine(c)
        else:
            known_sentence = False
            for sentence in self.knowledge:
                if sentence == Sentence(nearby_cells, count):
                    known_sentence == True
            if not known_sentence:
        ...
        for sentence1 in self.knowledge:
            for sentence2 in self.knowledge:
                ...
                if sentence1.cells.issuperset(sentence2.cells):
                    new_sentence = Sentence(sentence1.cells.difference(sentence2.cells), sentence1.count -
                        sentence2.count)
                    compared = False
                    for compare_sentence in self.knowledge:
                        if new_sentence == compare_sentence:
                            compared = True
                    if not compared:
    ...
    def make_random_move(self):
        ...
        if len(potential_moves) == 0 or self.moves_made == n_cells - 8:
            return None
        else:
